 ...
 */
package io.ucosty.kubernetes.annotator;
    ...
    public void propertyOrder(JDefinedClass clazz, JsonNode propertiesNode) {
        ...
        try {
            clazz.annotate(Buildable.class)
                    ...
                    .param("builderPackage", "io.ucosty.kubernetes.api.builder")
                    .annotationParam("inline", Inline.class)
                    .param("type", new JCodeModel()._class("io.ucosty.kubernetes.api.model.Doneable"))
    ...
    private void annotateMetatadataValidator(JDefinedClass clazz) {
        ...
        for (Map.Entry<String, JFieldVar> f : clazz.fields().entrySet()) {
            if (f.getKey().equals("metadata") && f.getValue().type().name().equals("ObjectMeta")) {
                try {
                    JAnnotationUse annotation = f.getValue().annotate(new JCodeModel()._class("io.ucosty.kubernetes.api.model.validators.CheckObjectMeta"));
